...
const String window_bilat_name = "Bilateral Filter";
int gauss_blur = 3, max_gauss = 9;
int bilat_sigma = 10, max_bilat_sigma = 150;
...
static void bilateral_filter_trackbar(int, void*)
{
    setTrackbarPos("Sigma", window_bilat_name, bilat_sigma);
}
...
int main(int argc, char* argv[])
{
    ...
    createTrackbar("Sigma", window_bilat_name, &bilat_sigma, max_bilat_sigma, bilateral_filter_trackbar);
    Mat frame, frame_HSV, frame_threshold, frame_blur, frame_bilat;
    while (true) {
        cap >> frame;
        if(frame.empty())
        {
        ...
        bilateralFilter(frame_blur, frame_bilat, -1, bilat_sigma, bilat_sigma);
        ...
        imshow(window_bilat_name, frame_bilat);
